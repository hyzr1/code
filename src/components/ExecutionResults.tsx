...
export default function ExecutionResults({result,submitted,onContinue}:{result:RunResult;submitted:boolean;onContinue?:()=>void}) {
 ...
 return <div className="execution-results"><div className="execution-verdict"><h2 className={result.ok?'result-pass':'result-fail'}>{result.ok&&!submitted?'Examples passed':submissionStatus(result)}</h2><span>{result.ms} ms</span><span>{result.results.filter(t=>t.passed).length}/{result.results.length} passed</span>{submitted&&<span className={`judge-status ${result.verified?'verified':'local'}`}>{result.verified?'Verified by Hyzr':'Checked locally'}</span>}</div>
