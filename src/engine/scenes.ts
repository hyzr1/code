...
const VISUALS: [RegExp, VisualKind][] = [
  ...
  [/algo\.(?:suffix-array-construction|suffix-automata)/, "hash"],
  [/algo\.(?:palindromic-trees|aho-corasick-automata)/, "hash"],
  [/ml\.(?:packaging-serving|latency-autoscaling|batching-caching)/, "system"],
  [/ml\.(?:online-batch-inference|safe-rollout)/, "system"],
  [/ml\.(?:monitoring-drift|online-experiments|versioning-lineage)/, "system"],
  ...
  [/ml\.(?:gaussian-processes|kernel-design|bayesian-optimization)/, "ml"],
  [/ml\.(?:causal-graphs|confounding)/, "graph"],
  [/ml\.(?:potential-outcomes|instrumental-variables)/, "decision"],
  [/ml\.(?:gpu-architecture|cuda-fundamentals)/, "system"],
  [/ml\.(?:custom-kernels|roofline)/, "system"],
  [/ml\.(?:operator-fusion|compilers|kernel-autotuning)/, "pipeline"],
  [/ml\.(?:three-dimensional-parallelism|communication-overlap)/, "system"],
  [/ml\.(?:fault-tolerant-training|frontier-utilization)/, "system"],
  [/ml\.(?:quantization-sparsity|paged-attention)/, "system"],
  [/ml\.(?:multi-token-decoding|edge-deployment)/, "system"],
  [/ml\.(?:tools-environments|planning-memory)/, "pipeline"],
  [/ml\.(?:multi-agent-orchestration|agent-evaluation)/, "pipeline"],
  [/ml\.(?:offline-rl|self-play|world-models)/, "ml"],
  [/ml\.(?:exploration-reward)/, "ml"],
  [/ml\.(?:inference-time-compute|continual-learning)/, "ml"],
  [/ml\.(?:long-term-memory|scaling-science)/, "ml"],
  [/ml\.(?:faithfulness|distribution-shift)/, "decision"],
  [/ml\.(?:superalignment|road-ahead)/, "decision"],
