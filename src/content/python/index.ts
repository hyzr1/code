...
const UNITS: UnitSpec[] = [
  ...
  },
  {
    ...
    model: "An f-string placeholder can carry a **format spec** after a colon: `{value:spec}`. It changes how the value is shown, never the value itself.\n\n`{price:.2f}` shows a float with two decimals. `{count:,}` groups thousands with commas. `{n:>8}` right-aligns in an eight-column field, `{n:<8}` left-aligns, and `{n:^8}` centers. `{ratio:.1%}` shows a fraction as a percentage. `{n:03d}` pads an integer to width three with leading zeros. These are what make numeric and tabular output line up and read cleanly.",
    example: `price = 3.5\ncount = 1234567\n\namount = f"{price:.2f}"     # "3.50"\ngrouped = f"{count:,}"      # "1,234,567"\npadded = f"{7:>4}"         # "   7"`,
    trace: "The colon begins the spec. `.2f` rounds the float to two decimals for display only. `,` inserts thousands separators. `>4` pads the value into a four-wide column, right-aligned. Each expression builds a new string, and `price` and `count` keep their exact values.",
    trap: "A format spec changes only the text, never the number: `f\"{price:.2f}\"` builds the string `\"3.50\"`, not a rounded float. And the type code matters—`f` for floats, `d` for integers, `%` for percentages—so pairing a spec with the wrong kind of value raises `ValueError`.",
    rule: "Put presentation in the format spec after the colon—decimals, thousands separators, width, alignment—so the underlying value stays exact.",
    recall: "In `f\"{price:.2f}\"`, what does `.2f` change, and what does it leave unchanged?",
    check: {
      question: "What does `f\"{1/3:.2f}\"` produce?",
      choices: ["\"0.33\" — rounded to two decimals for display", "the full \"0.3333333333333333\""],
      answer: 0,
      explanation: "The `.2f` spec formats the displayed text to two decimals. The underlying number is untouched; only the string is shorter.",
    },
  ...
  },
  {
    ...
    model: "Assignment can pull a sequence apart in one step: `a, b = pair` binds two names at once. A **starred target** is a name written with a leading star, and it absorbs the rest: it binds one value and gathers everything else into a list, as the example shows.\n\nThe same star **spreads** a sequence into a new one. Starring two lists joins them end to end, and double-starring two dictionaries merges them, where a repeated key keeps its later value. At a call site, a starred list argument is passed as positional arguments, and a double-starred dictionary is passed as keyword arguments.",
    example: `first, *rest = [10, 20, 30, 40]   # first=10, rest=[20, 30, 40]\ncombined = [*rest, 50]            # [20, 30, 40, 50]\ndefaults = {"a": 1, "b": 2}\nmerged = {**defaults, "b": 9}     # {"a": 1, "b": 9}`,
    trace: "The starred target gathers the leftover items into a list, so `first` holds one value and `rest` holds the remaining three. Spreading a list into a new list places its items inline. Merging two dictionaries copies the first and then applies the second, so a key present in both ends with the later value.",
    trap: "A merge keeps the last value for a repeated key, so order matters: the second dictionary overrides the first. And an unpacking may contain only one starred target, because two would be ambiguous about where the middle ends.",
    rule: "Use a starred target to split the front off a sequence, a star or double-star to combine sequences and mappings, and starred arguments to forward a list or dictionary into a call.",
    recall: "When two dictionaries are merged and both contain the same key, whose value wins?",
    check: {
      question: "What is `{**{\"a\": 1}, **{\"a\": 9}}`?",
      choices: ["`{\"a\": 9}` — a later key wins", "`{\"a\": 1}` — the first key wins"],
      answer: 0,
      explanation: "Merging copies left to right, so a repeated key keeps its last value. `b` overrides `a` in `{**a, **b}`.",
    },
