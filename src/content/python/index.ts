...
interface UnitSpec {
  ...
  requires?: string[];
  model: string;
  /** Optional hand-holding material for concepts that need more than one pass. */
  why?: string;
...
const UNITS: UnitSpec[] = [
  ...
  },
  {
    ...
    trace: "Line 1 binds `original` to a string that includes spaces at both ends. Line 2 calls `strip` and binds the returned string `\"Ada Lovelace\"` to a new name. Line 3 returns `\"ada lovelace\"`. Line 4 replaces the internal space and returns `\"ada-lovelace\"`. Line 5 joins the literal text `\"user:\"` with `username`, producing `\"user:ada-lovelace\"`. None of these calls changes the string bound to `original`.",
  ...
  },
  {
    ...
    trace: "The first assignment stores `10` in `score`, and the second binds `missing` to Python's special absence value. The next three comparisons produce Boolean results without changing `score`. The number `3` and the string `\"3\"` display similarly but are not equal values. `missing is None` is `True` because the name refers to that one special object. On the final line, the empty string acts as false and `not` reverses it to `True`.",
  ...
  },
  {
    ...
    trap: "`\",\".join([1, 2])` raises `TypeError`: `join` only accepts strings, so convert numbers first with `str`. And splitting the text `\"a  b\"` with no argument collapses the two spaces, while splitting that same text with the explicit separator `\" \"` keeps an empty string between them—choose the rule you actually want.",
  ...
  },
  {
    ...
    why: "So far, you could keep a bank account in a dictionary and write separate functions that change it. That works for a tiny program. As the program grows, however, you must remember which dictionary keys belong together and which functions are safe to use. A class gives that group a name and keeps the data beside the operations that protect it. Real Python libraries use this idea for files, dates, web requests, database records, and much more.",
    mentalPicture: "Picture a class as a cookie cutter and an instance as one cookie. The cutter describes the shape; it is not itself a cookie. Each cookie made from it is separate. In the same way, `Counter` describes what every counter can store and do, while `left` and `right` are two independent objects made from that description. The analogy is only a memory aid: a Python class is executable code, not a paper drawing.",
    model: "Start with four words that are easy to mix up. A **class** is code that defines a new kind of thing. An **object** is one value that exists while the program runs. An **instance** is an object made from a particular class. An **attribute** is a named value stored on an object.\n\nThe smallest useful class is easier to understand in two stages. First we describe the type:\n\n```python\nclass Counter:\n    def __init__(self, start):\n        self.value = start\n```\n\n`class Counter:` starts the definition. Nothing named `left` or `right` exists yet. `def __init__(self, start):` defines the setup method Python will call when a new `Counter` is created. `self.value = start` stores the starting number on that new object.\n\nNow we create an instance:\n\n```python\nleft = Counter(4)\n```\n\nPython makes a blank `Counter`, passes that new object into `__init__` as `self`, passes `4` as `start`, and then gives the finished object the name `left`. Afterward, `left.value` is `4`.\n\n`self` is not a magic global variable and it does not mean the class. It means *the particular instance receiving this method call*. You write `self` in the method definition, but the caller normally does not pass it explicitly.",
    example: `class Counter:\n    def __init__(self, start=0):\n        self.value = start\n\n    def increment(self):\n        self.value += 1\n        return self.value\n\nleft = Counter(4)\nright = Counter(100)\n\nprint(left.increment())\nprint(left.value)\nprint(right.value)`,
    trace: "Read it in execution order. Python first creates the class and learns its two methods. `left = Counter(4)` creates one object; inside `__init__`, `self` is that new left object, so its `value` becomes `4`. `right = Counter(100)` creates a different object whose `value` becomes `100`. Then `left.increment()` makes `self` refer to `left`, changes only `left.value` from `4` to `5`, and returns `5`. The three printed values are therefore `5`, `5`, and `100`. The right counter did not move. That is what **per-instance state** means.",
    secondExample: `class BankAccount:\n    def __init__(self, owner, balance=0):\n        if balance < 0:\n            raise ValueError("balance cannot start below zero")\n        self.owner = owner\n        self.balance = balance\n\n    def deposit(self, amount):\n        if amount <= 0:\n            raise ValueError("deposit must be positive")\n        self.balance += amount\n        return self.balance\n\naccount = BankAccount("Maya", 20)\nprint(account.deposit(5))\nprint(account.balance)`,
    secondTrace: "This example shows why a class is more than a fancy dictionary. `BankAccount` owns the rules for valid account state. Its constructor refuses a negative starting balance. Its `deposit` method refuses zero or a negative amount before changing the balance. A caller does not need to know every internal step; it uses the small public interface `deposit`. The output is `25` and then `25`. Keeping validation beside the data makes invalid states harder to create by accident.",
    checkpoint: "Before the answer appears, predict this: if `a = Counter(2)` and `b = Counter(9)`, what are `a.value` and `b.value` after `a.increment()`? Explain what `self` referred to during that call.",
    checkpointAnswer: "`a.increment()` passes the object named `a` as `self`. Because `a.value` and `b.value` were stored on two different objects during two different calls to `__init__`, changing `a.value` does not change `b.value`.",
    trap: "Do not put a changeable per-object value directly in the class body. In the broken example below, every team shares the same list:\n\n```python\nclass Team:\n    members = []\n\nfirst = Team()\nsecond = Team()\nfirst.members.append(\"Maya\")\nprint(second.members)  # ['Maya'] -- surprising\n```\n\n`members` belongs to the class, so both instances find the same list. Fix it by writing `self.members = []` inside `__init__`; then each call creates a fresh list for one instance. Also remember that every ordinary instance method needs `self` as its first parameter.",
    rule: "A class defines a type; calling the class creates an instance. `__init__` establishes that instance's valid starting state. `self` is the particular instance receiving a method call. Store each object's own changeable data on `self`, and expose a few clear methods that protect the object's rules.",
  ...
  {
    id: "composition", module: "py.m4", title: "Inheritance and composition, from zero", goal: "Understand both ways classes can work together and choose the safer relationship.", kind: "pattern", requires: ["classes"],
    why: "One class is enough for a small example, but real programs contain many classes. Those classes must share jobs without becoming a tangled pile where changing one thing breaks five others. Inheritance and composition are two different ways to connect them. They are not advanced decorations; they are promises about how the objects relate. Choosing the wrong promise makes code confusing even when every line runs.",
    mentalPicture: "Use two questions. **Is-a?** A dog is an animal, so inheritance might fit. **Has-a?** A car has an engine, so composition fits. Do not choose by asking, 'Can I steal some code from this class?' Choose by asking what relationship will still be true when the program changes.",
    model: "First, inheritance. A **parent class** (also called a base class) describes behavior shared by a family of types. A **child class** (also called a subclass) names the parent inside parentheses and receives that behavior.\n\n```python\nclass Animal:\n    def speak(self):\n        return \"some sound\"\n\nclass Dog(Animal):\n    pass\n\ndog = Dog()\nprint(dog.speak())\n```\n\n`Dog(Animal)` means `Dog` inherits from `Animal`. `Dog` has no `speak` method of its own, so Python looks in its parent and finds `Animal.speak`. The output is `some sound`.\n\nA child can **override** a method by defining a method with the same name:\n\n```python\nclass Cat(Animal):\n    def speak(self):\n        return \"meow\"\n```\n\nNow `Cat().speak()` returns `meow`, while `Dog().speak()` still uses the parent version. Python starts its method search on the actual object's class and walks up to the parent only if needed.\n\nSometimes the child wants to keep the parent's work and add one step. `super()` means, 'call the parent version for this same object.'\n\n```python\nclass NamedAnimal(Animal):\n    def __init__(self, name):\n        self.name = name\n\nclass NamedDog(NamedAnimal):\n    def __init__(self, name):\n        super().__init__(name)\n        self.tricks = []\n```\n\n`NamedDog(\"Pip\")` runs the child constructor. `super().__init__(name)` lets `NamedAnimal` store the name on the new dog, and the next line gives that dog its own tricks list. Forgetting the `super()` call would leave `self.name` unset.\n\nInheritance makes a serious promise: callers should be able to use the child anywhere they can use the parent without getting a nasty surprise. That is what **substitutable** means. A `Dog` can stand in for an `Animal` because it still answers `speak()` and keeps the meaning of that operation.",
    example: `class Engine:\n    def start(self):\n        return \"engine started\"\n\nclass Car:\n    def __init__(self, engine):\n        self.engine = engine\n\n    def start(self):\n        return self.engine.start()\n\nengine = Engine()\ncar = Car(engine)\nprint(car.start())`,
    trace: "This is **composition**. `Car` does not inherit from `Engine`, because a car is not a kind of engine. A car **has an** engine. `engine = Engine()` creates the helper object. `Car(engine)` passes that object into `Car.__init__`, which stores it as `self.engine`. When `car.start()` runs, the car delegates the smaller job to `self.engine.start()`. 'Delegate' simply means 'ask another object to do the job it owns.' The output is `engine started`.",
    secondExample: `class MemoryRepository:\n    def get(self, report_id):\n        return {"id": report_id, "score": 92}\n\nclass TextFormatter:\n    def format(self, data):\n        return f"Report {data['id']}: {data['score']}"\n\nclass ReportService:\n    def __init__(self, repository, formatter):\n        self.repository = repository\n        self.formatter = formatter\n\n    def build(self, report_id):\n        data = self.repository.get(report_id)\n        return self.formatter.format(data)\n\nservice = ReportService(MemoryRepository(), TextFormatter())\nprint(service.build(7))`,
    secondTrace: "Now the same idea appears in professional-looking code. A repository is just an object responsible for getting data. A formatter is just an object responsible for turning data into text. `ReportService` has one of each. Its `build` method coordinates the steps: get report `7`, then format the returned dictionary. The output is `Report 7: 92`. Later, you could pass a database repository or a JSON formatter without rewriting `ReportService`. Tests can pass tiny fake collaborators. That easy swapping is the main benefit of composition.",
    checkpoint: "Classify each relationship before moving on: a `Square` and a `Shape`; a `Computer` and a `Keyboard`. Which one might use inheritance, and which one should use composition? Say is-a or has-a out loud.",
    checkpointAnswer: "Inheritance says 'this child is a kind of the parent and can safely stand in for it.' Composition says 'this object has another object and asks it to do a focused job.' A car has an engine, so composition is honest; saying a car is an engine would be false.",
    trap: "Do not inherit merely to grab a convenient method. For example, `class UserNames(list)` may seem handy because it gets `append`, `sort`, and dozens of other list methods for free. But it also promises that `UserNames` behaves like an unrestricted list everywhere. Callers could insert numbers, clear it, or reverse it even if those actions break your rules. A `UserNames` object that *holds* a private list and exposes only `add_name` makes the smaller, clearer promise. Inheritance is safe when the child truly preserves the parent's meaning, not just when the syntax saves lines.",
    rule: "Use inheritance for a genuine is-a relationship where every child can safely replace the parent. Use composition for a has-a relationship: store a collaborator on `self` and delegate its focused job. When you only want code reuse or easy swapping, composition is usually the clearer and safer default.",
  ...
  },
  {
    ...
    model: "A good test is a tiny, readable story: it names one behavior, sets up the smallest state that shows it, does one action, and checks one visible result. When it fails, the name alone should tell you what broke. Here is the complete tiny system under test and its tests:\n\n```python\ndef clamp(value, low, high):\n    return max(low, min(value, high))\n\ndef test_clamp_caps_a_value_above_the_range():\n    assert clamp(12, 0, 10) == 10\n\ndef test_clamp_leaves_an_in_range_value_alone():\n    assert clamp(4, 0, 10) == 4\n```\n\nThe function *names* describe the contract, so a failure reads like a sentence. And notice these test *behavior*, not *how* it is coded — if you later rewrite `clamp`, the tests still pass because the behavior is unchanged.\n\n**The real skill is choosing inputs.** A handful of well-chosen cases finds far more bugs than dozens of similar happy examples. Cover the **partitions** where bugs hide:\n- **empty** (an empty list, `\"\"`)\n- **single** item\n- **ordinary** case\n- **boundary** (the exact edge — `0`, the max, off-by-one)\n- **invalid** input\n\n**One trap:** do not compute a test's expected answer the same way the code does — if the code is wrong, your test repeats the mistake and 'passes' for the wrong reason. Work out the expected value *independently* from the specification.",
    example: `def clamp(value, low, high):\n    return max(low, min(value, high))\n\ndef test_clamp_caps_a_value_above_the_range():\n    assert clamp(12, 0, 10) == 10\n\ndef test_clamp_leaves_an_in_range_value_alone():\n    assert clamp(4, 0, 10) == 4\n\ntest_clamp_caps_a_value_above_the_range()\ntest_clamp_leaves_an_in_range_value_alone()`,
  ...
  },
  {
    ...
    example: `def has_duplicate(values):\n    seen = set()\n    for value in values:\n        if value in seen:\n            return True\n        seen.add(value)\n    return False\n\nprint(has_duplicate([4, 2, 4]))`,
    trace: "Call `has_duplicate([4, 2, 4])`. The loop visits at most `n` values. At `4`, the set lookup misses and `4` is added. At `2`, the lookup misses and `2` is added. At the final `4`, membership succeeds and the function returns `True`. Each pass does expected O(1) set work, so total expected time is O(n). The set can hold up to `n` values, so extra space is O(n). Re-scanning the earlier list for every value would make each check O(n) and the whole solution O(n²).",
  ...
  },
  {
    ...
    trace: "For `\"([)]\"`: `(` and `[` are pushed, so `stack` is `['(', '[']`. Then `)` arrives; its match is `(`, but the top of the stack is `[` — mismatch — so it returns `False`. That is correct: `)` should have closed the most recent opener, which was `[`. For the properly nested example, every closer matches the current top, each pop succeeds, and the stack ends empty, so it returns `True`.",
  ...
  },
  {
    ...
    model: "The **merge-intervals pattern** combines overlapping ranges. The key move is to **sort by start**.\n\nAfter sorting, a new interval can only overlap the most recent merged one. Compare with that frontier instead of scanning every earlier range.\n\n```python\ndef merge(intervals):\n    intervals.sort(key=lambda pair: pair[0])   # sort by start\n    merged = [intervals[0]]\n    for start, end in intervals[1:]:\n        last_start, last_end = merged[-1]\n        if start <= last_end:                  # overlaps the current frontier\n            merged[-1] = [last_start, max(last_end, end)]   # extend it\n        else:\n            merged.append([start, end])         # gap -> start a new interval\n    return merged\n\nprint(merge([[1, 3], [2, 6], [8, 10]]))   # [[1, 6], [8, 10]]\n```\n\nThe last merged interval reaches farthest right. If a new interval does not touch it, the new interval cannot touch an earlier one.",
  ...
  },
  {
    ...
    rule: "Define exactly what an item on the stack is *waiting for* (for example, 'a greater value to its right') and the event that resolves it — then each item is pushed and popped once for O(n).",
  ...
  },
  {
    ...
    model: "**Union-find** (also called disjoint-set) answers one question fast: *are these two things in the same group?* Picture people forming friend circles. Every circle has one representative called its **root**. Two people are connected when `find` reaches the same root for both.\n\nTwo lists store the forest. `parent[x]` is the next node above `x`; a root points to itself. `size[root]` is how many nodes belong to that root's group.\n\n```python\nparent = list(range(5))\nsize = [1] * 5\n\ndef find(x):\n    while x != parent[x]:\n        parent[x] = parent[parent[x]]\n        x = parent[x]\n    return x\n\ndef union(a, b):\n    root_a = find(a)\n    root_b = find(b)\n    if root_a == root_b:\n        return False\n    if size[root_a] < size[root_b]:\n        root_a, root_b = root_b, root_a\n    parent[root_b] = root_a\n    size[root_a] += size[root_b]\n    return True\n```\n\n`find` climbs parent links. Its shortcut line points a node to its grandparent while climbing; this is **path compression**. `union` first finds roots because whole groups—not arbitrary nodes—must be connected. It then attaches the smaller root under the larger root and updates the winner's size; this is **union by size**. Both tricks keep the trees shallow. The amortized cost per operation grows so slowly that it is effectively constant for practical inputs.",
    example: `parent = list(range(5))\nsize = [1] * 5\ndef find(x):\n    while x != parent[x]:\n        parent[x] = parent[parent[x]]\n        x = parent[x]\n    return x\ndef union(a, b):\n    root_a, root_b = find(a), find(b)\n    if root_a == root_b:\n        return False\n    if size[root_a] < size[root_b]:\n        root_a, root_b = root_b, root_a\n    parent[root_b] = root_a\n    size[root_a] += size[root_b]\n    return True\nunion(0, 1)\nunion(1, 2)\nprint(find(0) == find(2))\nprint(find(0) == find(4))`,
    trace: "Start with `parent = [0, 1, 2, 3, 4]` and every size equal to `1`; each node is a one-person group. `union(0, 1)` finds roots `0` and `1`. Their sizes tie, so the code keeps `0` as the winner, sets `parent[1] = 0`, and makes `size[0] = 2`. Next, `union(1, 2)` finds root `0` for node `1` and root `2` for node `2`. The two-person group is larger, so node `2` attaches under root `0` and `size[0]` becomes `3`. Now `find(0)` and `find(2)` both return `0`, so the first output is `True`. Node `4` still points to itself, so the second output is `False`.",
  ...
  },
  {
    ...
    trace: "Start from `a` with distance zero and queue `[a]`.\n\nPop `a`. Reach `b` and `c` for the first time, give both distance one, and queue `[b, c]`. Pop `b`; reach `d`, give it distance two, and queue `[c, d]`.\n\nPop `c`. Its neighbor `d` already has a distance, so skip it. Pop `d`; it has no neighbors. The result is `{a: 0, b: 1, c: 1, d: 2}`. First discovery is shortest because each queue layer uses one more edge than the previous layer.",
  ...
  },
  {
    ...
    trace: "Keep the 3 largest of `[3, 2, 1, 5, 6, 4]`. Push 3, 2, 1: the heap contains 1, 2, and 3, with `1` at the front. Push 5: the heap has 4 items, too many, so pop the front `1`; the survivors are 2, 3, and 5. Push 6: pop front `2`; the survivors are 3, 5, and 6. Push 4: pop front `3`; the survivors are 4, 5, and 6. Every time the pile grew past 3, we removed its smallest — so the survivors are always the biggest values seen so far.",
  ...
  },
  {
    ...
    trace: "Call `search(0)` with an empty path. First skip item `1`, then skip item `2`; at the base case, record `[]`.\n\nBack up one level, choose `2`, record `[2]`, then pop it to restore the empty path. Return to the root, choose `1`, and explore again. That branch records `[1]` and `[1, 2]`.\n\nFinally pop `1`, restoring the empty path. The answers are `[[], [2], [1], [1, 2]]`. Every pop hands the next branch the exact state it expects.",
  ...
  },
  {
    ...
    why: "Dynamic programming is not a bag of mysterious table templates. It is a way to stop solving the same smaller question again and again. Once you can say exactly what one saved answer means, the base cases and recurrence become ordinary code. This lesson begins with a tiny table before compressing state in the harder example.",
    mentalPicture: "Imagine a worksheet where each new answer may reuse completed boxes. Give every box a precise label, fill the boxes whose answers are already known, and fill each new box only from boxes that are ready. The label is the state definition; the rule connecting boxes is the recurrence.",
    model: "**Dynamic programming** applies when different paths reach the same smaller question. Solve each distinct question once, store the answer, and reuse it. Start with climbing stairs: from each step you may climb one or two steps. How many different ways reach step `n`?\n\nName the state before writing the loop: `ways[i]` means *the number of ways to stand exactly on step i*. There is one way to stand at step zero—start there—and one way to reach step one. To reach any later step, the final move came from one step below or two steps below. That gives the recurrence `ways[i] = ways[i - 1] + ways[i - 2]`.\n\n```python\ndef climb(n):\n    if n < 0:\n        return 0\n    ways = [0] * (n + 1)\n    ways[0] = 1\n    for step in range(1, n + 1):\n        ways[step] += ways[step - 1]\n        if step >= 2:\n            ways[step] += ways[step - 2]\n    return ways[n]\n\nprint(climb(4))  # 5\n```\n\nThe five routes to step four are `1+1+1+1`, `1+1+2`, `1+2+1`, `2+1+1`, and `2+2`. The table does not guess them; it counts them from already solved smaller states. A DP solution needs four named parts: the state meaning, base cases, transition, and final answer location. Only after those are clear should you compress memory.",
    checkpoint: "For the climbing-stairs table, write `ways[0]` through `ways[5]`. Which two saved values create `ways[5]`, and what does the number in that box mean in a full sentence?",
    checkpointAnswer: "The values are `1, 1, 2, 3, 5, 8`. `ways[5]` comes from `ways[4] + ways[3]`, or `5 + 3`. The full meaning is: there are eight different sequences of one-step and two-step moves that land exactly on step five.",
  ...
  },
  {
    ...
    trace: "Use capacity two and accesses `A B A C A`. `A` misses, then `B` misses, producing cache order `[A, B]`.\n\nThe next `A` hits. Move it to the recent end, giving `[B, A]`. Then `C` misses and briefly produces `[B, A, C]`. Evict the oldest key, `B`, leaving `[A, C]`.\n\nThe final `A` hits because the earlier hit refreshed it. Move it to the end for `[C, A]`. Total misses: three. Without hit refresh, `A` would have been evicted and the final access would miss.",
  ...
  {
    id: "loop-control", module: "py.m1", title: "While, break, continue, and loop else", goal: "Choose condition-controlled repetition, prove that it makes progress, and distinguish normal exhaustion from an early exit.", kind: "mental-model", requires: ["loops", "branching", "aggregation-tools"],
    model: "A `for` loop is the clearest choice when an iterable already defines the next value. A `while` loop is for repetition controlled by a condition: retry until ready, consume while data remains, or advance pointers until they meet. Python checks the condition **before** every iteration, so a false starting condition runs the body zero times.\n\nEvery `while` loop needs a **progress argument**: name the state that changes and explain why that change must eventually make the condition false. Without one, the loop can run forever.\n\nTwo statements alter ordinary iteration:\n- `continue` skips the rest of the current iteration and checks for the next one. Update any required progress state before continuing.\n- `break` exits the nearest loop immediately. It does not exit the surrounding function.\n\nA loop's `else` block runs only when the loop ends **without `break`**. For a search, that means the entire search space was exhausted without finding a match. A `return` also leaves the function, so it does not continue into the loop's `else`.\n\nUse `else` only when that exhaustion meaning is clearer than a flag variable. It is valid on both `for` and `while` loops, but unfamiliar readers should not have to decode it when an early `return` states the contract more directly.",
    example: `def find_index(values, target):
    index = 0
    while index < len(values):
        ...
        return -1
    return index`,
    trace: "Start with `index = 0`. Before each pass, check that the index is still inside the list. A mismatch increments the index, which is the loop's progress step. A match executes `break`, so Python skips the `else` block and returns that index. If the condition becomes false because every index was checked, no `break` occurred; the `else` block returns `-1`. The two exits therefore encode found versus exhausted.",
    trap: "The most dangerous `continue` bug skips the statement that makes progress. In a `while` loop, if an invalid item executes `continue` before the index advances or new input is read, Python checks the same state forever. Put the progress update before `continue`, or restructure the loop so progress has one obvious home.",
    rule: "Use `for` for iterable-driven repetition and `while` for condition-driven repetition. Before trusting `while`, state what changes, why it moves toward termination, and what `break`, exhaustion, and errors each mean.",
    recall: "When does a loop's `else` run, and what progress proof prevents a `while` loop from becoming infinite?",
    checks: [
      {
        question: "When does the `else` block attached to a loop run?",
        choices: ["When the loop finishes without executing `break`", "After every iteration", "Only when the loop body raises an exception"],
        answer: 0,
        why: ["Correct. Normal exhaustion reaches loop `else`; an executed `break` skips it.", "The `else` is attached to the loop as a whole, not to each iteration.", "An unhandled exception leaves the loop and does not run its `else` block."],
        explanation: "Loop `else` distinguishes normal exhaustion from an early `break`.",
      },
      {
        question: "What must you be able to explain for every `while` loop?",
        choices: ["Which state changes and why the condition must eventually become false", "Why `while` is always faster than `for`", "Why the body must run at least once"],
        answer: 0,
        why: ["Correct. That progress argument is the termination proof.", "Neither syntax is inherently faster; choose the control model that matches the problem.", "A pre-checked `while` body may correctly run zero times."],
        explanation: "A condition-controlled loop is safe only when its changing state guarantees eventual termination.",
      },
      {
        question: "What does `continue` do?",
        choices: ["Skips the rest of this iteration and begins the next check", "Exits the nearest loop", "Restarts the entire function"],
        answer: 0,
        why: ["Correct. Any required state update must happen before that jump.", "`break` exits the nearest loop.", "`continue` stays inside the same loop and function call."],
        explanation: "Continue advances control to the loop's next iteration, which can be dangerous if progress was skipped.",
      },
    ],
    prompt: "Return the index of the first occurrence of `target` in `values`, or `-1` when it is absent. Use a `while` loop with an explicit progress step.", fn: "find_index", starter: `def find_index(values, target):
    pass`, solution: `def find_index(values, target):
    index = 0
    while index < len(values):
        if values[index] == target:
            return index
        index += 1
    return -1`,
    tests: [t("found first", "assert fn([4, 7, 9], 4) == 0"), t("found later", "assert fn([4, 7, 9], 9) == 2"), t("absent", "assert fn([4, 7, 9], 2) == -1"), t("empty", "assert fn([], 2) == -1", true), t("first duplicate", "assert fn([1, 3, 1], 1) == 0", true)], minutes: 6,
  },
];
...
const VOCABULARY: Record<string, Array<[term: string, meaning: string]>> = {
  ...
  "loop-control": [["while loop", "repetition that continues while a condition remains true"], ["break", "a statement that exits the nearest loop immediately"], ["continue", "a statement that skips the rest of the current iteration"], ["loop else", "a block that runs only when a loop ends without `break`"]],
  ...
  composition: [["parent class", "a class whose behavior can be inherited"], ["child class", "a class that inherits from a parent"], ["override", "giving a child its own version of an inherited method"], ["super", "a way to call the parent implementation for the current object"], ["composition", "building behavior by storing and coordinating smaller objects"], ["inheritance", "creating a class by extending another class"], ["dependency", "an object or service another component needs to do its work"], ["substitutable", "safe to use in every place where the promised parent type is expected"]],
...
type LessonGuidance = Pick<UnitSpec,
  "why" | "mentalPicture" | "secondExample" | "secondTrace" | "checkpoint" | "checkpointAnswer"
>;

/** Extra, concept-specific teaching passes for lessons that need more than one example. */
const LESSON_GUIDANCE: Partial<Record<string, LessonGuidance>> = {
  "first-function": {
    why: "Functions stop you from copying the same steps and give a useful operation a name. Nearly every Python program is a collection of small functions calling one another. If defining, calling, printing, and returning are blurry now, every later topic will feel harder than it is, so this lesson slows down at those boundaries.",
    mentalPicture: "A function is like a small machine stored under a name. The `def` statement builds the machine; it does not feed anything through it. A call such as `greet(\"Ada\")` starts one run. Parameters are labeled input slots, arguments are the actual values placed in those slots, and `return` is the output sent back to the caller.",
    secondExample: `def double(number):\n    result = number * 2\n    return result\n\nfirst = double(3)\nsecond = double(first)\nprint(second)`,
    secondTrace: "Python reaches `def` and creates `double`; its body still has not run. `double(3)` begins the first call, binds local `number` to `3`, computes local `result` as `6`, and returns `6`. Assignment gives that returned value the name `first`. Then `double(first)` starts a completely new call with `number` equal to `6`, so it returns `12`. The final output is `12`. The local names from the first call do not keep running between calls.",
    checkpoint: "Without running it, decide what `answer` becomes in `answer = double(5)`. Then decide what changes if `double` prints `result` but has no `return` statement.",
    checkpointAnswer: "With `return result`, the call produces `10`, so `answer` becomes `10`. If the function only prints `10`, the screen still shows `10`, but the call itself returns `None`, so `answer` becomes `None`. Displaying a value and sending a value back are different jobs.",
  },
  fstrings: {
    why: "Programs constantly turn data into messages, labels, logs, and reports. F-strings keep the sentence and its values together, which is easier to read than a chain of string additions. They also avoid the common error of trying to add a number directly to text.",
    mentalPicture: "Read an f-string as a sentence with small windows cut into it. Ordinary characters stay exactly as written. Python opens each `{...}` window, evaluates the expression inside, turns that result into display text, and places it into the surrounding sentence.",
    secondExample: `item = "notebook"\nprice = 4.5\nquantity = 3\ntotal = price * quantity\n\nmessage = f"{quantity} {item}s cost \${total:.2f}"\nprint(message)`,
    secondTrace: "Python evaluates the placeholders from left to right. `{quantity}` becomes `3`; `{item}` becomes `notebook`; and `{total:.2f}` reads `total` as `13.5` and formats it with exactly two digits after the decimal point. The output is `3 notebooks cost $13.50`. Formatting changes only how the value appears in this string—`total` is still the numeric value `13.5` afterward.",
    checkpoint: "Predict the exact value of `f\"{2 + 3} apples\"`. Is the result a number or a string? Does evaluating an ordinary variable inside braces change that variable?",
    checkpointAnswer: "Python evaluates `2 + 3` first, inserts `5`, and produces the string `\"5 apples\"`. The whole f-string is text. Merely reading a variable or expression inside braces does not change it unless that expression itself performs a change.",
  },
  "text-split": {
    why: "Input rarely arrives in the exact shape your program wants. A user gives one sentence, a file gives one line, or a server gives comma-separated text; your code usually needs individual pieces. `split` crosses from one string to a list, and `join` crosses back. This pair appears in cleanup, file parsing, URLs, commands, and interview problems.",
    mentalPicture: "Imagine cutting a paper strip at each separator. `split` makes the pieces. `join` lays pieces in a row and puts one chosen separator only in the gaps between them. With four pieces there are three gaps, so `join` inserts three separators—not one before the first or after the last.",
    secondExample: `line = "red,green,,blue"\nparts = line.split(",")\nprint(parts)\n\nrebuilt = " | ".join(parts)\nprint(rebuilt)`,
    secondTrace: "Because an explicit comma separator is used, Python cuts at every comma. Two neighboring commas have nothing between them, so the list contains an empty string: `['red', 'green', '', 'blue']`. Joining all four pieces places ` | ` into all three gaps. The empty third piece adds no text, so the result is `red | green |  | blue`. This differs from no-argument `split()`, which treats runs of whitespace as one boundary.",
    checkpoint: "Predict both results: `\"  a   b  \".split()` and `\"a,,b\".split(\",\")`. Explain why only one result contains an empty string.",
    checkpointAnswer: "No-argument `split()` collapses surrounding and repeated whitespace, so the first result is `['a', 'b']`. An explicit comma means every comma is a real cut; the neighboring commas enclose an empty piece, so the second result is `['a', '', 'b']`.",
  },
  slicing: {
    why: "Slicing is the compact language Python uses for taking part of a string, list, tuple, and many data-library objects. The syntax looks small, but off-by-one mistakes are common unless you can translate every slice into the exact indices it selects.",
    mentalPicture: "Put boundaries between the items: boundary `0` is before the first item, boundary `1` is after it, and so on. `items[start:stop]` keeps everything between two boundaries. The start boundary is included; the stop boundary is not. The step tells Python how far to jump between selected positions.",
    secondExample: `numbers = [0, 1, 2, 3, 4, 5]\nprint(numbers[1:5:2])\nprint(numbers[-3:])\nprint(numbers[4:1:-1])\nprint(numbers[:])`,
    secondTrace: "`[1:5:2]` begins at index `1`, stays before index `5`, and jumps by two, so it selects `[1, 3]`. `[-3:]` starts three positions from the end and gives `[3, 4, 5]`. `[4:1:-1]` starts at index `4` and walks backward while staying above the excluded stop index `1`, giving `[4, 3, 2]`. `[:]` copies the outer list. The copy is new, but any nested objects inside would still be shared.",
    checkpoint: "For `letters = ['a', 'b', 'c', 'd', 'e']`, predict `letters[1:4]`, `letters[:2]`, and `letters[3:0:-1]`. Point to the excluded stop boundary each time.",
    checkpointAnswer: "The results are `['b', 'c', 'd']`, `['a', 'b']`, and `['d', 'c', 'b']`. A positive slice stops before `4` or `2`. The negative-step slice walks backward and stops before index `0`, so it includes indices `3`, `2`, and `1` but not `0`.",
  },
  collections: {
    why: "Counting, grouping, and queueing are so common that Python ships carefully tested containers for them. Knowing these tools saves code, but the real skill is recognizing the shape of the job: tally values, collect items under keys, or remove items in arrival order.",
    mentalPicture: "Treat the three containers as three desks. `Counter` has one tally sheet per value. `defaultdict(list)` has labeled bins and creates a fresh bin when a new label first appears. `deque` is a line with a door at both ends, so adding or removing at either end is quick.",
    secondExample: `from collections import deque\n\nwaiting = deque(["Ava", "Ben"])\nwaiting.append("Cam")\nnext_person = waiting.popleft()\n\nprint(next_person)\nprint(list(waiting))`,
    secondTrace: "The deque begins with Ava at the left or front and Ben behind her. `append('Cam')` adds Cam at the right or back. `popleft()` removes and returns Ava without shifting Ben and Cam through memory. The outputs are `Ava` and `['Ben', 'Cam']`. A list can model the same idea, but repeated `pop(0)` gets slower as the list grows because all remaining references must move left.",
    checkpoint: "Choose a tool for each job: count error codes, group students by classroom, and process tasks first-in-first-out. Explain the shape of the operation, not merely the tool's name.",
    checkpointAnswer: "Use `Counter` for error-code tallies, `defaultdict(list)` to collect many students under each classroom key, and `deque` for the FIFO task queue. The choices follow the required operations: counting, grouping, and fast removal from the front.",
  },
  itertools: {
    why: "Many problems ask for every pair, every arrangement, or every cross-product. Hand-written nested loops are easy to get wrong and often build a huge list before it is needed. `itertools` gives those patterns names and produces one result at a time.",
    mentalPicture: "An itertools object is a conveyor belt, not a warehouse. It knows how to produce the next item when asked, but it does not necessarily store all future items. This laziness saves memory, and it also explains why consuming the same iterator a second time finds nothing left.",
    secondExample: `from itertools import groupby\n\nrecords = [("blue", 2), ("red", 1), ("blue", 5)]\nrecords.sort(key=lambda row: row[0])\n\nfor color, group in groupby(records, key=lambda row: row[0]):\n    values = [row[1] for row in group]\n    print(color, values)`,
    secondTrace: "The original blue records are separated, so grouping immediately would create two blue runs. Sorting by the same color key first makes equal keys adjacent. `groupby` then yields a color plus a one-shot iterator for that consecutive group. The loop consumes each group and prints `blue [2, 5]` and `red [1]`. Sorting is not a hidden feature of `groupby`; it is your responsibility when you want global groups rather than runs.",
    checkpoint: "With the items `['A', 'B', 'C']`, how many length-two results come from `combinations` and how many from `permutations`? List them and explain the difference.",
    checkpointAnswer: "Combinations produces three unordered choices: `AB`, `AC`, and `BC`. Permutations produces six ordered arrangements: `AB`, `AC`, `BA`, `BC`, `CA`, and `CB`. Reversing a pair makes a new permutation but not a new combination.",
  },
  debugging: {
    why: "Professional developers spend a large part of their time understanding code that does not behave as expected. Debugging is not a talent for staring at code until inspiration arrives. It is a repeatable investigation that turns a large mystery into one small, testable question.",
    mentalPicture: "Imagine a row of dominoes. The last fallen domino is the visible crash, but it may not be the cause. Walk from the first known-good state toward the symptom and find the first domino that fell the wrong way. That first wrong state transition is usually where the useful explanation lives.",
    secondExample: `def average(numbers):\n    total = 0\n    for number in numbers:\n        total = number\n    return total / len(numbers)\n\nprint(average([2, 4, 6]))`,
    secondTrace: "The expected result is `4`, but the function returns `2`. Shrink the case to `[2, 4]`; now the expected result is `3`, but the function returns `2`. Trace `total`: it starts at `0`, becomes `2`, then becomes `4`. The first divergence occurs on the first loop pass because a running total should become old total plus the number, not merely the number. Change only `total = number` to `total += number`, rerun the small case, and then rerun the full tests.",
    checkpoint: "Suppose the final total is wrong after a loop. What exact values would you record on each iteration, and why is that better than adding ten unrelated print statements?",
    checkpointAnswer: "Record the loop index or current item, the total before the update, and the total after it. Those values test one hypothesis: the state transition that updates the total is wrong. Targeted evidence tells you where expected and actual behavior first separate; unrelated logging creates noise without answering a question.",
  },
  performance: {
    why: "Fast code is useful only when it solves the right problem correctly. A benchmark tells you whether a change helped, while a profiler tells you where time is actually going. This prevents days of polishing a line that contributes almost nothing to the user's wait.",
    mentalPicture: "Think of a trip with ten road segments. Making a ten-second segment twice as fast saves five seconds; fixing the two-hour traffic jam saves far more. A profiler measures the segments. Big-O warns how a segment changes when the input becomes ten or a thousand times larger.",
    secondExample: `import timeit\n\nvalues = list(range(10_000))\nlist_time = timeit.timeit(lambda: 9_999 in values, number=1_000)\nvalue_set = set(values)\nset_time = timeit.timeit(lambda: 9_999 in value_set, number=1_000)\n\nprint(set_time < list_time)`,
    secondTrace: "Both expressions answer the same question, so correctness is held constant. `timeit` repeats each lookup enough times to reduce clock noise. A list membership test may scan thousands of values, while a set lookup uses hashing and is usually constant-time on average. The final output should normally be `True`, but the lesson is not to promise an exact ratio: measure on representative data and compare distributions when the decision matters.",
    checkpoint: "A profiler says parsing takes 80 percent of a request and formatting takes 2 percent. You can make either part twice as fast. Which change has the larger ceiling, and approximately how much total time can each save?",
    checkpointAnswer: "Halving parsing can save about 40 percent of the total request. Halving formatting can save only about 1 percent. Optimize the measured bottleneck first; even a perfect rewrite of a tiny segment has a tiny effect on the whole program.",
  },
  caching: {
    why: "Caching can turn repeated expensive work into a quick lookup, but it also makes the program remember old answers. That trade is valuable only when the same request repeats and the answer is determined completely by the cache key.",
    mentalPicture: "A cache is a labeled answer notebook. Before solving a problem, look up its label. If the answer is present, reuse it; that is a hit. Otherwise solve it, write the answer under that label, and return it; that is a miss. A bounded LRU notebook erases the answer unused for the longest time when space runs out.",
    secondExample: `from functools import lru_cache\n\n@lru_cache(maxsize=2)\ndef price_with_tax(price, rate):\n    print("calculating")\n    return price * (1 + rate)\n\nprint(price_with_tax(10, 0.1))\nprint(price_with_tax(10, 0.1))\nprint(price_with_tax.cache_info())`,
    secondTrace: "The first call is a cache miss, so the body prints `calculating`, computes `11.0`, and stores it under the argument pair `(10, 0.1)`. The second identical call is a hit: it returns the stored `11.0` without running the body, so `calculating` appears only once. `cache_info()` exposes hits, misses, current size, and capacity, which lets you verify that real calls reuse entries instead of assuming they do.",
    checkpoint: "Should you cache `current_temperature(city)` forever if it reads a live sensor? What is missing from the key or policy, and what stale behavior could the caller see?",
    checkpointAnswer: "No. The same city argument can have a different correct answer later, so the arguments do not fully determine the result. The cache needs an expiration policy or a time or version component in its key, or it should not be used. Otherwise callers may receive an old temperature indefinitely.",
  },
  method: {
    why: "Interview problems feel like riddles when you believe the solution must suddenly appear in your head. The derivation loop replaces that pressure with small questions you can answer in order. It also gives the interviewer evidence: you understand the slow solution, the repeated work, the chosen data structure, and why the faster version is correct.",
    mentalPicture: "Think like a detective, not a magician. The brute-force solution is the honest first witness. Its repeated work leaves fingerprints. Those fingerprints point to the data structure that remembers exactly what was being searched or recomputed. The invariant is your proof that the new shortcut never changes the answer.",
    secondExample: `def has_pair_sum(numbers, target):\n    seen = set()\n    for number in numbers:\n        needed = target - number\n        if needed in seen:\n            return True\n        seen.add(number)\n    return False\n\nprint(has_pair_sum([4, 1, 8, 3], 9))`,
    secondTrace: "This is the runnable result of the derivation. Before each loop pass, `seen` contains exactly the earlier numbers. At `4`, the needed partner is `5`, which is absent, so store `4`. At `1`, needed `8` is absent, so store `1`. At `8`, needed `1` is already in `seen`, proving an earlier value plus the current value makes `9`; return `True`. Each value is visited once, and each expected set lookup is constant time, so the total expected time is O(n).",
    checkpoint: "For `[2, 7, 4]` and target `6`, list `number`, `needed`, and `seen` before each lookup. Why must the lookup happen before adding the current number?",
    checkpointAnswer: "The states are `(2, 4, {})`, then `(7, -1, {2})`, then `(4, 2, {2, 7})`; the final lookup succeeds. Looking before adding prevents one value from pairing with itself unless the input truly contains that value twice. The invariant says `seen` contains only positions already visited.",
  },
  hashing: {
    why: "A hash table is useful when the slow part of a solution is repeatedly asking, 'Have I seen this?' or 'What information belongs to this key?' The table spends extra memory to make those questions fast. That time-for-space trade powers counts, duplicate detection, joins, caches, and many interview solutions.",
    mentalPicture: "Picture labeled drawers. Computing a hash helps Python find the area for a key; equality confirms the exact drawer. The key is the label and the value is the fact stored behind it. The useful design question is not 'Should I use a dictionary?' but 'What question must this label answer in one lookup?'",
    secondExample: `def two_sum(numbers, target):\n    index_by_value = {}\n    for index, number in enumerate(numbers):\n        needed = target - number\n        if needed in index_by_value:\n            return [index_by_value[needed], index]\n        index_by_value[number] = index\n    return None\n\nprint(two_sum([2, 7, 11, 15], 9))`,
    secondTrace: "The dictionary means: each key is a number already visited, and its value is that number's index. At index `0`, number `2` needs `7`; `7` is not stored, so save `2: 0`. At index `1`, number `7` needs `2`; key `2` exists with index `0`, so return `[0, 1]`. Checking before storing prevents using the current position twice. The expected runtime is O(n), with O(n) extra space in the worst case.",
    checkpoint: "In the two-sum dictionary, what exactly does a key mean, what does its value mean, and what is guaranteed to be stored before processing index `i`?",
    checkpointAnswer: "A key is a number from a position already visited, its value is an index where that number appeared, and before index `i` the dictionary contains only positions smaller than `i`. That meaning is the invariant that makes a successful complement lookup prove two distinct positions form the answer.",
  },
  "two-pointers": {
    why: "A nested loop checks many pairs even after order has made most of them impossible. Two pointers turn that order into evidence: every movement throws away a whole group of candidates at once. The pattern matters only when you can explain why the discarded group cannot contain an answer.",
    mentalPicture: "Imagine two fingers on opposite ends of a sorted row of cards. Their sum gives feedback. If it is too small, moving the right finger left would only make it smaller, so the only useful direction is moving the left finger toward larger cards. The sorted order is the fact that makes the move safe.",
    secondExample: `def reverse_in_place(values):\n    left = 0\n    right = len(values) - 1\n    while left < right:\n        values[left], values[right] = values[right], values[left]\n        left += 1\n        right -= 1\n\nletters = ["a", "b", "c", "d"]\nreverse_in_place(letters)\nprint(letters)`,
    secondTrace: "Initially the unprocessed region is the whole list. Swap `a` and `d`; those two outside positions are now correct forever. Move both pointers inward. Swap `b` and `c`; now every position is correct and the pointers meet. The output is `['d', 'c', 'b', 'a']`. The invariant is that everything outside `[left, right]` is already in its final reversed position.",
    checkpoint: "For sorted `[1, 4, 6, 9]` with target `10`, trace the first sum. Which pointer moves, and exactly which pairs does that movement rule out?",
    checkpointAnswer: "The first sum is `1 + 9 = 10`, so the answer is found immediately and no pointer moves. If the target were `12`, that sum would be too small; advancing left would discard every pair using `1`, because `1 + 9` was already the largest possible pair containing `1`.",
  },
  stack: {
    why: "Some problems cannot be solved by looking only at the current item; they need the most recent unfinished item. A stack stores exactly that history. It appears in undo systems, function calls, browser navigation, expression parsing, depth-first search, and nested brackets.",
    mentalPicture: "Picture a stack of cafeteria trays. You place a tray on top and remove the top tray first. Reaching an older tray requires resolving everything above it. Nested code has the same rule: the most recently opened bracket or function call must finish before the older one can finish.",
    secondExample: `def simplify_path(parts):\n    stack = []\n    for part in parts:\n        if part == "..":\n            if stack:\n                stack.pop()\n        elif part not in ("", "."):\n            stack.append(part)\n    return "/" + "/".join(stack)\n\nprint(simplify_path(["home", "notes", "..", "photos"]))`,
    secondTrace: "Start empty. `home` is a real folder, so push it. Push `notes`. The `..` instruction means go back from the most recent folder, so pop `notes`. Push `photos`. Joining the remaining stack produces `/home/photos`. The stack is useful because going back always cancels the most recently entered unresolved folder.",
    checkpoint: "Trace the stack for the bracket text `([])`. What is on the stack after each character, and why is an empty final stack required?",
    checkpointAnswer: "After `(` the stack is `['(']`; after `[` it is `['(', '[']`; `]` matches and pops `[`, leaving `['(']`; `)` matches and pops `(`, leaving `[]`. An item left at the end would be an opener that never received a closer, so the text would not be balanced.",
  },
  intervals: {
    why: "Calendar events, bookings, numeric ranges, video clips, and memory regions all become interval problems. Sorting turns a messy collection of relationships into a left-to-right story where only the latest merged frontier can still overlap the next range.",
    mentalPicture: "Draw each interval as a line segment. After sorting by where segments begin, the merged output behind you is finished except for its last segment—the one reaching farthest right. The next segment either begins before that frontier ends and joins it, or begins after it and starts a new island.",
    secondExample: `def can_attend_all(meetings):\n    meetings = sorted(meetings, key=lambda meeting: meeting[0])\n    for previous, current in zip(meetings, meetings[1:]):\n        if current[0] < previous[1]:\n            return False\n    return True\n\nprint(can_attend_all([[9, 10], [13, 14], [9.5, 11]]))`,
    secondTrace: "Sorting gives `[9, 10]`, `[9.5, 11]`, `[13, 14]`. Compare neighboring meetings. The second starts at `9.5`, before the first ends at `10`, so they overlap and the function returns `False`. Touching endpoints are allowed here because the comparison uses `<`; a meeting ending at `10` and another starting at `10` do not overlap under this chosen half-open rule.",
    checkpoint: "Merge `[1, 4]`, `[2, 3]`, and `[6, 8]` by hand. Why must the merged end use `max(current_end, new_end)` instead of always taking `new_end`?",
    checkpointAnswer: "The result is `[[1, 4], [6, 8]]`. The interval `[2, 3]` is completely contained inside `[1, 4]`. Replacing the end with `3` would incorrectly shrink the covered range; `max(4, 3)` preserves the farthest frontier at `4`.",
  },
  "binary-search": {
    why: "Binary search is not just 'find a number in a sorted list.' It finds the boundary where a yes-or-no condition changes from false to true. That same idea handles insertion positions, minimum feasible capacities, first bad versions, and many optimization questions.",
    mentalPicture: "Imagine a row colored red and then green, with exactly one boundary. Testing the middle tells you which half cannot contain the first green position. Keep the half that still might contain the boundary. The interval shrinks until one position remains.",
    secondExample: `def contains(values, target):\n    low = 0\n    high = len(values)\n    while low < high:\n        mid = (low + high) // 2\n        if values[mid] < target:\n            low = mid + 1\n        else:\n            high = mid\n    return low < len(values) and values[low] == target\n\nprint(contains([1, 3, 5, 7], 5))\nprint(contains([1, 3, 5, 7], 4))`,
    secondTrace: "The loop first finds the lower-bound position: the first index whose value is at least the target. That position is useful even when the target is absent, so one final check is required. For `5`, the boundary is index `2` and `values[2] == 5`, giving `True`. For `4`, the boundary is also index `2`, but `values[2]` is `5`, giving `False`. Finding an insertion point and proving exact membership are separate steps.",
    checkpoint: "For `[1, 3, 5, 7]` and target `4`, trace `(low, high, mid)` until the loop ends. Why does the algorithm return index `2` even though `4` is absent?",
    checkpointAnswer: "Start `(0, 4, 2)`; value `5` is not less than `4`, so high becomes `2`. Then `(0, 2, 1)`; value `3` is less than `4`, so low becomes `2`. The range is empty at boundary `2`. Lower bound asks where `4` could be inserted while keeping order, not whether `4` exists.",
  },
  "prefix-sums": {
    why: "When many questions ask for totals over different ranges of the same data, adding each range from scratch repeats almost all the work. A prefix array pays for one complete pass up front and answers every later range with two lookups and one subtraction.",
    mentalPicture: "Picture mile markers that store total distance traveled from the start. The distance between marker `left` and marker `right` is total-to-right minus total-to-left. Both totals include the shared journey before `left`; subtraction cancels that shared part.",
    secondExample: `def build_prefix(values):\n    prefix = [0]\n    for value in values:\n        prefix.append(prefix[-1] + value)\n    return prefix\n\ndef range_sum(prefix, left, right):\n    return prefix[right] - prefix[left]\n\nprefix = build_prefix([5, 2, 7, 1])\nprint(range_sum(prefix, 0, 2))\nprint(range_sum(prefix, 1, 4))`,
    secondTrace: "The prefix array is `[0, 5, 7, 14, 15]`; each position stores the sum before that boundary. Range `[0, 2)` is `prefix[2] - prefix[0]`, or `7 - 0 = 7`. Range `[1, 4)` is `prefix[4] - prefix[1]`, or `15 - 5 = 10`. The leading zero makes a range beginning at index zero obey the same formula, avoiding a special case.",
    checkpoint: "For values `[4, -2, 6]`, build the full prefix array including the leading zero, then compute the sum of slice `[1:3]` using one subtraction.",
    checkpointAnswer: "The prefix array is `[0, 4, 2, 8]`. Slice `[1:3]` contains `-2` and `6`, so use `prefix[3] - prefix[1] = 8 - 4 = 4`. The boundary indices match Python's half-open slice notation exactly.",
  },
  "monotonic-stack": {
    why: "A plain stack remembers unfinished items. A monotonic stack adds an order rule so one new value can finish several waiting items at once. That is why an inner `while` can still be linear: no index can be removed more times than it was added.",
    mentalPicture: "Imagine people standing in a line waiting to see the first taller person who arrives after them. Keep unresolved people on a stack. When a taller person arrives, they answer the question for every shorter person exposed at the top, one by one, until the remaining top is at least as tall.",
    secondExample: `def days_until_warmer(temperatures):\n    answer = [0] * len(temperatures)\n    stack = []\n    for day, temperature in enumerate(temperatures):\n        while stack and temperatures[stack[-1]] < temperature:\n            earlier_day = stack.pop()\n            answer[earlier_day] = day - earlier_day\n        stack.append(day)\n    return answer\n\nprint(days_until_warmer([70, 68, 72]))`,
    secondTrace: "Day `0` at 70 waits on the stack. Day `1` at 68 is not warmer than 70, so it also waits. Day `2` at 72 is warmer than 68, so pop day `1` and record a wait of `1`; it is also warmer than 70, so pop day `0` and record `2`. Day `2` never sees a future warmer day, so its answer stays `0`. The result is `[2, 1, 0]`.",
    checkpoint: "Why can the total number of `while`-loop pops never exceed the number of input items, even if one hot day removes many waiting days?",
    checkpointAnswer: "Each index is pushed exactly once. After an index is popped, it never returns to the stack. One iteration may do many pops, but across the entire function there can be at most `n` pushes and `n` pops, which makes the total work O(n).",
  },
  "union-find": {
    why: "Repeated graph searches are wasteful when the only question is whether two nodes belong to the same connected group. Union-find maintains those groups as edges arrive, which is useful for network connections, account merging, cycle detection, and Kruskal's minimum-spanning-tree algorithm.",
    mentalPicture: "Every group is a small upside-down family tree with a representative at the top. `find` follows parents to the representative. `union` connects two representatives. Path compression shortens routes during a search; union by size prevents a small tree from becoming the parent of a much larger one.",
    checkpoint: "After merging `0` with `1` and then `1` with `2`, why is `parent[2]` allowed to point to `0` even though the request named node `1`? What fact identifies the whole group?",
    checkpointAnswer: "The request names any member, but `find(1)` first discovers the group's root. Union connects roots, not the raw arguments. All nodes with the same root belong to one component, so the representative returned by `find`—not a direct parent link—is the group's identity.",
  },
  heap: {
    why: "Sorting is wasteful when you need only the next highest-priority item or a small best-k set. A heap maintains just enough order to expose the minimum after every update. Priority queues for schedulers, graph algorithms, simulations, and streaming rankings use this exact trade.",
    mentalPicture: "A heap is a tournament podium, not a sorted line. The winner is guaranteed at the front, but the remaining competitors are arranged only enough to find the next winner cheaply after the first is removed. Looking sideways in the internal list does not reveal second, third, and fourth place in sorted order.",
    secondExample: `import heapq\n\ntasks = []\nheapq.heappush(tasks, (3, "send report"))\nheapq.heappush(tasks, (1, "fix outage"))\nheapq.heappush(tasks, (2, "review code"))\n\nwhile tasks:\n    priority, name = heapq.heappop(tasks)\n    print(priority, name)`,
    secondTrace: "Tuples compare from left to right, so the numeric priority decides which task is smallest. Popping repeatedly prints the outage first, then code review, then the report. The heap is not kept as one fully sorted list between pops; each pop repairs only the heap property in O(log n). Including the task name second also gives Python a deterministic tie-breaker when priorities match.",
    checkpoint: "You must keep the five largest values from a million-value stream. Why does a min-heap of size five discard the correct item whenever a new candidate arrives?",
    checkpointAnswer: "The heap contains the five current winners, and its root is the smallest winner—the weakest member of that set. After adding a new candidate, if there are six items, removing the smallest leaves exactly the five largest seen so far. The invariant holds after every input value.",
  },
  "api-contracts": {
    why: "An API is where separately written code meets. The caller cannot read your mind, so names, valid inputs, success shapes, and failure shapes must be stable promises. A clear contract prevents every deeper function from defending itself against every possible bad value.",
    mentalPicture: "Treat the API boundary like airport security. Untrusted input is checked once before entering the trusted area. Inside the trusted area, the rest of the system can rely on stated facts. A missing ticket, a fake ticket, and a valid flight with zero available seats are three different outcomes and should not be blurred together.",
    secondExample: `def create_user(payload):\n    if "email" not in payload:\n        return {"ok": False, "error": "email is required"}\n    email = payload["email"]\n    if not isinstance(email, str) or "@" not in email:\n        return {"ok": False, "error": "email is invalid"}\n    return {"ok": True, "user": {"email": email.lower()}}\n\nprint(create_user({"email": "ADA@EXAMPLE.COM"}))`,
    secondTrace: "The boundary first distinguishes a missing field from a present but malformed field. Only after both guards does the trusted path normalize the email. The success response always has `ok: True` and a `user`; failures always have `ok: False` and an `error`. The output contains the lowercase address. In a web service these shapes would pair with documented HTTP status codes, but the stable distinction exists even in a plain Python function.",
    checkpoint: "Classify three search outcomes: no `query` field was sent; `query` is a list instead of text; a valid text query matches zero rows. Why should the caller be able to distinguish all three?",
    checkpointAnswer: "They are missing input, malformed input, and a successful empty result. The first two mean the request contract was not satisfied and should report a client error. The third is a valid success whose data is an empty collection. A caller may ask the user to fix input for errors but simply display 'no results' for the valid empty case.",
  },
  "capacity-estimation": {
    why: "A system-design answer needs enough arithmetic to reveal what will break first. Requests per second estimate compute load; bytes per second estimate network load; bytes retained over time estimate storage. The numbers may be rough, but every unit and assumption must be visible so another engineer can challenge it.",
    mentalPicture: "Follow one event through three clocks. How many events arrive each second? How many bytes must cross the network each second? How many bytes remain after a day, month, or year? Average, peak, replication, and headroom are separate multipliers—write them separately instead of hiding them inside one impressive number.",
    secondExample: `def traffic_bounds(events_per_day, bytes_per_event, peak_factor=4):\n    seconds_per_day = 86_400\n    average_per_second = events_per_day / seconds_per_day\n    peak_bytes_per_second = average_per_second * peak_factor * bytes_per_event\n    storage_per_day = events_per_day * bytes_per_event\n    return average_per_second, peak_bytes_per_second, storage_per_day\n\nrate, peak_bandwidth, daily_storage = traffic_bounds(8_640_000, 2_000)\nprint(rate)\nprint(peak_bandwidth)\nprint(daily_storage)`,
    secondTrace: "The event rate is `100` per second on average. At a four-times peak and 2,000 bytes per event, incoming bandwidth is roughly `800,000` bytes per second, before protocol overhead or replication. One day of raw retained data is `17,280,000,000` bytes, about 17.28 decimal gigabytes. Replication by three and 30 days of retention would multiply that storage again. Keeping the units beside each line makes missing factors visible.",
    checkpoint: "If average traffic is 200 requests per second, peak is five times average, and you want 50 percent safety headroom, what capacity do you plan for? State each multiplication and the final unit.",
    checkpointAnswer: "Peak traffic is `200 requests/second × 5 = 1,000 requests/second`. Fifty percent headroom multiplies that by `1.5`, producing planned capacity of `1,500 requests per second`. The peak factor and safety factor solve different uncertainties, so keep them as separate assumptions.",
  },
  dataclasses: {
    why: "Many objects are records whose main job is carrying a few named values. A dataclass keeps that intention visible and generates routine methods correctly. It is still a real class: you can validate, add methods, freeze it, and control individual fields when the defaults are not safe.",
    mentalPicture: "A normal class is an empty form plus every instruction for filling, printing, and comparing it. A dataclass lets you list the form's fields, and Python writes those routine instructions. You still decide the meaning and rules; the decorator removes repetition, not design.",
    secondExample: `from dataclasses import dataclass, field\n\n@dataclass\nclass Playlist:\n    name: str\n    songs: list[str] = field(default_factory=list)\n\nfirst = Playlist("Focus")\nsecond = Playlist("Gym")\nfirst.songs.append("Rain")\n\nprint(first.songs)\nprint(second.songs)`,
    secondTrace: "`default_factory=list` stores the list-making function, not one shared list. Each call to `Playlist(...)` calls that factory and receives a fresh empty list. Adding `Rain` changes only `first.songs`; the outputs are `['Rain']` and `[]`. Writing `songs = []` would try to share one changeable default, so dataclasses reject it instead of quietly creating the bug.",
    checkpoint: "When should you choose a dataclass instead of a dictionary or a behavior-heavy normal class? Explain what Python generates and what design work remains yours.",
    checkpointAnswer: "Choose a dataclass when a stable record has known named fields and mostly value-like behavior. Python can generate initialization, representation, and equality. You still choose field meanings, validation, mutability, methods, and safe factories for changeable defaults. A loose, unpredictable mapping may still be a dictionary; a type with complex invariants may deserve a hand-designed class.",
  },
  asyncio: {
    why: "Network programs spend much of their life waiting. Asyncio lets one thread use that waiting time for another task, but only at explicit suspension points. Understanding when work starts is the difference between real overlap and code that merely contains the word `await` while still running serially.",
    mentalPicture: "One cook manages several pots. The cook starts a pot, and while it simmers, works on another. `await` is the moment a task says, 'I cannot continue yet; let another ready task use the cook.' CPU-heavy work that never awaits keeps the cook occupied and delays every pot.",
    secondExample: `import asyncio\n\nasync def fetch(name, delay):\n    print(f"start {name}")\n    await asyncio.sleep(delay)\n    print(f"finish {name}")\n    return name\n\nasync def main():\n    results = await asyncio.gather(\n        fetch("A", 0.02),\n        fetch("B", 0.01),\n    )\n    print(results)\n\nasyncio.run(main())`,
    secondTrace: "`gather` schedules both coroutine calls before waiting for the combined result. Both print their start messages. Each reaches `asyncio.sleep`, suspends, and gives the event loop another ready task. B's shorter wait finishes first, then A, but `gather` returns results in the original argument order, so the final list is `['A', 'B']`. The overlap helps because sleep represents waiting, not CPU work.",
    checkpoint: "What is the ordering difference between `a = await fetch('A')` followed by `b = await fetch('B')`, and `await asyncio.gather(fetch('A'), fetch('B'))`?",
    checkpointAnswer: "Two direct awaits are serial: A starts and finishes before B starts. `gather` schedules both, so when one suspends during I/O, the other can make progress. The total time is near the slower wait rather than the sum of both waits, assuming waiting dominates and the external service allows the concurrency.",
  },
  parallelism: {
    why: "Threads, processes, and asyncio solve different bottlenecks. Picking by fashion can make a program slower or introduce race bugs. The useful decision begins with evidence: is time spent executing Python instructions, waiting for I/O, or waiting for an external service with its own limit?",
    mentalPicture: "Concurrency is one person keeping several errands moving while some are waiting. Parallelism is several people doing work at the same instant. Threads share one room and its objects; processes work in separate rooms and must copy or send anything they share.",
    secondExample: `from concurrent.futures import ThreadPoolExecutor\nimport time\n\ndef wait_for_result(name):\n    time.sleep(0.02)\n    return name.upper()\n\nwith ThreadPoolExecutor(max_workers=2) as pool:\n    results = list(pool.map(wait_for_result, ["a", "b"]))\n\nprint(results)`,
    secondTrace: "The two worker threads call a waiting-heavy function. While one thread sleeps, it releases the interpreter lock and the other can run, so the waits overlap. `pool.map` preserves input order and returns `['A', 'B']`. This does not prove threads speed up CPU-heavy Python loops; for that workload the global interpreter lock prevents Python bytecode from running on two threads at once, so processes are the usual choice after measurement.",
    checkpoint: "Choose a first tool for each measured bottleneck: 5,000 open network sockets, 20 ordinary blocking HTTP calls, and four long pure-Python calculations on an eight-core machine. State the tradeoff.",
    checkpointAnswer: "Use asyncio for thousands of sockets when async libraries are available; use a thread pool for a modest number of blocking I/O calls; and consider a process pool for the CPU-heavy calculations. Asyncio and threads overlap waiting, while processes can use multiple cores but pay serialization and startup costs.",
  },
  arguments: {
    why: "A function signature is the front door to a piece of code. It should make valid calls easy and confusing calls difficult. Defaults, named arguments, and starred parameters are not tricks; they let the signature communicate which inputs are required, optional, positional, or important enough to name.",
    mentalPicture: "Picture a labeled order form. Ordinary parameters have boxes that may be filled in order or by label. A default is a box already filled unless the caller replaces it. One-star extras collect leftover unlabeled items, two-star extras collect leftover labeled items, and parameters after a bare star must be filled by label.",
    secondExample: `def connect(host, *, timeout=5, secure=True):\n    scheme = "https" if secure else "http"\n    return f"{scheme}://{host} (timeout {timeout})"\n\nprint(connect("example.com"))\nprint(connect("example.com", timeout=10, secure=False))`,
    secondTrace: "The bare `*` is a divider, not a collected value. `host` may be positional, but `timeout` and `secure` are **keyword-only**, so the call must name them. The first call uses both defaults and returns an HTTPS label with timeout 5. The second replaces both by name and returns an HTTP label with timeout 10. `connect('example.com', 10)` raises `TypeError` instead of silently guessing what `10` means.",
    checkpoint: "Why might an API force `timeout` and `secure` to be keyword-only even though Python could accept them by position?",
    checkpointAnswer: "A call like `connect('example.com', 10, False)` hides what `10` and `False` mean and becomes fragile if parameters are reordered. `connect('example.com', timeout=10, secure=False)` documents itself at the call site. Keyword-only parameters trade a few characters for clarity and safer evolution.",
  },
  scope: {
    why: "Names can be reused in different functions without colliding because every call has its own local scope. Closures deliberately preserve a small part of an enclosing scope. Understanding the lookup order prevents accidental globals and explains why two functions can remember different private values.",
    mentalPicture: "When Python needs a name, it searches nested boxes from the inside out: the current function, wrapping functions, the module, then built-ins. A closure carries a live link to an enclosing box. It does not take a frozen photograph of every value at the moment the inner function was written.",
    secondExample: `def make_adders():\n    adders = []\n    for amount in [1, 10, 100]:\n        def add(number, saved_amount=amount):\n            return number + saved_amount\n        adders.append(add)\n    return adders\n\nadd_one, add_ten, add_hundred = make_adders()\nprint(add_one(5), add_ten(5), add_hundred(5))`,
    secondTrace: "A closure normally looks up the shared loop variable later, after the loop has finished at `100`. The default parameter fixes that trap because each `def` evaluates `amount` immediately and stores it as that function's own `saved_amount` default. The three calls therefore print `6 15 105`. This is a deliberate snapshot; ordinary closure lookup remains a live link to the enclosing binding.",
    checkpoint: "If the inner `add` used `return number + amount` with no default parameter, what would all three functions return for input `5`, and why?",
    checkpointAnswer: "All three would return `105`. They close over the same `amount` binding, not three copied values. The loop finishes with that binding equal to `100`, and the functions read it only when called. Capturing `amount` as a default evaluates and saves each iteration's value during function creation.",
  },
};

function body(unit: UnitSpec): string {
  const guidance = LESSON_GUIDANCE[unit.id];
  ...
  const vocabulary = (VOCABULARY[unit.id] ?? [])
    ...
    .join("\n");
  const whyText = unit.why ?? guidance?.why;
  const mentalPictureText = unit.mentalPicture ?? guidance?.mentalPicture;
  const secondExampleText = unit.secondExample ?? guidance?.secondExample;
  const secondTraceText = unit.secondTrace ?? guidance?.secondTrace;
  const checkpointText = unit.checkpoint ?? guidance?.checkpoint;
  const checkpointAnswerText = unit.checkpointAnswer ?? guidance?.checkpointAnswer;
  const why = whyText ? `## Why this matters\n\n${whyText}\n\n` : "";
  const mentalPicture = mentalPictureText
    ? `## A picture to keep in your head\n\n${mentalPictureText}\n\n`
    : "";
  const secondWalkthrough = secondExampleText && secondTraceText
    ? `## See the same idea in a second example\n\n\`\`\`python\n${secondExampleText}\n\`\`\`\n\n${secondTraceText}\n\n`
    : "";
  const checkpointAnswer = checkpointText && checkpointAnswerText
    ? `## Pause and predict\n\n${checkpointText}\n\n## Check your explanation\n\n${checkpointAnswerText}\n\n`
    : "";
...
${why}${mentalPicture}## The idea, step by step
...
${secondWalkthrough}${checkpointAnswer}## A mistake to avoid
...
const ORDER_WITHIN_MODULE: Record<string, string[]> = {
  ...
  "py.m1": ["fstrings", "booleans", "branching", "lists", "text-split", "slicing", "loops", "iteration-tools", "aggregation-tools", "loop-control", "names", "functions"],
