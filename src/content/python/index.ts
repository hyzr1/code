...
const UNITS: UnitSpec[] = [
  ...
  },
  {
    ...
    trace: "`first, *rest = [10, 20, 30, 40]`: `first` takes the first value `10`, and the starred `rest` gathers everything left over into a list, `[20, 30, 40]`.\n\n`[*a, *b]` with `a = [1, 2]` and `b = [3, 4]` drops each list's items inline into a new list, giving `[1, 2, 3, 4]` — a tidy way to join them.\n\n`{**defaults, \"size\": \"L\"}` copies `defaults` first, then applies `\"size\": \"L\"`. Since `size` was already there, the later value wins, so the result is `{'size': 'L', 'color': 'red'}`.",
    trap: "An unpacking may have only one starred name — two would be ambiguous about where the middle ends. And when merging dictionaries with `**`, a key that appears in both keeps the value from the *last* one, so `{**a, **b}` lets `b` override `a`. Order matters.",
    rule: "Use a single starred name to peel the front (or ends) off a sequence into a list, and `*`/`**` to spread lists and dicts into new ones — remembering that a repeated dict key keeps its last value.",
    recall: "In `first, *rest = [10, 20, 30, 40]`, what do `first` and `rest` hold, and when two dicts are merged with `**` and share a key, whose value wins?",
    checks: [
      {
