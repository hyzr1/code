...
const UNITS: UnitSpec[] = [
  ...
  },
  {
    ...
    model: "A **class** is a blueprint for making your own kind of object — a new type that bundles together some **data** and the **actions** that go with it. For example, a `Counter` bundles a number with the ability to increase it.\n\n```python\nclass Counter:\n    def __init__(self, start=0):   # runs when you create one\n        self.value = start         # per-counter data\n\n    def increment(self):           # an action\n        self.value += 1\n        return self.value\n\nc = Counter()          # make one (an 'instance')\nprint(c.increment())   # 1\nprint(c.increment())   # 2\n```\n\nThree ideas to hold onto:\n- **`__init__`** runs automatically when you create an instance. Its job is to establish valid starting data.\n- **`self`** is the instance a method is working on. Calling `c.increment()` makes `c` the `self` inside that method. You write the parameter, but Python supplies the argument.\n- Data stored on `self` is **per-instance** — each `Counter` carries its own `value`.\n\nSo a class lets you create many objects that each hold their own data and share the same behavior.",
  ...
  },
  {
    ...
    trace: "Start `parent = [0,1,2,3,4]`, with everyone their own leader.\n\nFor `union(0, 1)`, find leaders 0 and 1, then set `parent[0]=1`. The state becomes `[1,1,2,3,4]`, so 1 leads {0,1}.\n\nNext, `union(1, 2)` joins the whole {0,1} group to 2. Setting `parent[1]=2` changes the list to `[1,2,2,3,4]`. Now 2 leads {0,1,2}. `find(0)` follows 0 to 1 to 2 and returns 2. `find(2)` also returns 2, so the first comparison is True. `find(4)` returns 4, so the second comparison is False.",
